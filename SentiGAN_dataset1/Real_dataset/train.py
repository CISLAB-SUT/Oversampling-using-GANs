...
parser = argparse.ArgumentParser(description='SentiGAN')
parser.add_argument('--train', default=False, action='store_true', help='whether to train or not')
parser.add_argument('--generate', default=False, action='store_true', help='generate')
...
imdb_file_txt = dataset_path + "sentineg.txt"
imdb_file_id = dataset_path + "sentineg.id"
...
# sst_pos_file_txt = dataset_path + 'yelp2_pos.txt'
# sst_pos_file_id = dataset_path + 'yelp2_pos.id'
sst_neg_file_txt = dataset_path + 'ssentineg.txt'
sst_neg_file_id = dataset_path + 'sentineg.id'
...
def generate_samples(sess, trainable_model, generated_num, output_file, vocab_list, if_log=False, epoch=0):
    ...
    if if_log:
        ...
        if epoch == 0:
            mode = 'w+'
        with open(eval_text_file, mode, encoding="utf8") as fout:
...
def generate_my_samples(sess, trainable_model, generated_num, output_file, vocab_list, if_log=False, epoch=0):
    # Generate Samples
    generated_samples = []
    for _ in range(int(generated_num)):
        generated_samples.extend(trainable_model.generate(sess))
...
def load_emb_data(emb_dict_file):
    ...
    word_list = []
    item = 0
    with open(emb_dict_file, 'r', encoding="utf8") as f:
        lines = f.readlines()
        for line in lines:
            word = line.strip()
            word_dict[word] = item
...
def main():
    ...
    for _ in range(10):   # 10
        ...
        dis_data_loader.load_train_data([sst_neg_file_id], [negative_file])
        for _ in range(3):
...
def generate():
    ...
    with tf.Session() as sess:
        saver = tf.train.import_meta_graph('./save/ckpt/model.ckpt.meta')
        saver.restore(sess, "./save/ckpt/model.ckpt")
        generate_my_samples(sess, generator, 223200, my_file, vocab_list)
    #
    #
    #
    ...
    #     new_saver.restore(sess, tf.train.latest_checkpoint('./save/ckpt/'))
