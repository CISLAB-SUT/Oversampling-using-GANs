...
class BasicInstructor:
    ...
    def _save(self, phase, epoch):
        ...
        if epoch == 100:
            save_sample_path = cfg.save_samples_root + 'samples100.txt'
            samples = self.gen.sample(223200, cfg.batch_size)
            write_tokens(save_sample_path, tensor_to_tokens(samples, self.idx2word_dict))

        if epoch == 150:
            save_sample_path = cfg.save_samples_root + 'samples150.txt'
            samples = self.gen.sample(223200, cfg.batch_size)
            write_tokens(save_sample_path, tensor_to_tokens(samples, self.idx2word_dict))

        if epoch == 170:
            save_sample_path = cfg.save_samples_root + 'samples170.txt'
            samples = self.gen.sample(223200, cfg.batch_size)
            write_tokens(save_sample_path, tensor_to_tokens(samples, self.idx2word_dict))

        if epoch==200:
            save_sample_path = cfg.save_samples_root + 'samples200.txt'
            samples = self.gen.sample(223200, cfg.batch_size)
            write_tokens(save_sample_path, tensor_to_tokens(samples, self.idx2word_dict))
